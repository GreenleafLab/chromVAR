...
#' @export
get_motif_matches <- function(motif, seqs, 
                              nucfreqs = get_nuc_freqs(seqs), 
                              min.score = "90%", 
                              max.matches = length(seqs)/5){
  ...
  forward <- Biostrings::matchPWM(log(motif/nucfreqs),tmpseq, with.score=T, 
                                  min.score = min.score)
  reverse <- Biostrings::matchPWM(log(Biostrings::reverseComplement(motif)/nucfreqs),
                                  tmpseq, 
                                  with.score = T, 
                                  min.score = min.score)
  ...
  if (length(forward_matches)>0 | length(reverse_matches)>0){
    ...
    score_mat <- rbind(cbind(IRanges::subjectHits(forward_matches),
                             S4Vectors::mcols(forward[IRanges::queryHits(forward_matches)])$score),
                       cbind(IRanges::subjectHits(reverse_matches),
                             S4Vectors::mcols(reverse[IRanges::queryHits(reverse_matches)])$score))
    colnames(score_mat) <- c("id","score")
    max_scores <- plyr::ddply(as.data.frame(score_mat),"id",
                              plyr::summarize,max_score = max(score))
    if (nrow(max_scores) > max.matches){
      max_scores <- max_scores[which(max_scores$max_score >= quantile(max_scores$max_score,
                                                                      1 - (max.matches/nrow(max_scores)))),]
    }
    return(max_scores)
  } else{
...
get_motifs <- function(species = "Hsapiens", dataSource = "JASPAR"){
  ...
  if (length(dataSource)==1){
    ...
    data_indices <- do.call(union,
                            lapply(dataSource,
                                   function(x) grep(x,S4Vectors::values(MotifDb::MotifDb)$dataSource)))
  } else{
...
#' @export
get_motif_indices <- function(motifs, peaks, 
                              genome =  BSgenome.Hsapiens.UCSC.hg19::BSgenome.Hsapiens.UCSC.hg19, 
                              BPPARAM = BiocParallel::bpparam(), 
                              min.score = "90%",
                              max.matches = length(peaks)/5, 
                              min.matches = 50){
  ...
  #find motif matches
  motif_dfs <- BiocParallel::bplapply(motifs, 
                                      get_motif_matches, 
                                      seqs, 
                                      nucFreqs, 
                                      min.score, 
