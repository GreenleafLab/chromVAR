...
get_counts_from_10x_beds <- function(beds, peaks, paired, colData = NULL) {
  ...
  print(length(beds))
  # for each bed file
  # if 10x, the output is a list (bed) of lists (cells)
  results <- lapply(beds, function(i) {
   ...
   # if no colData was given, set colData$depths to depths and set names of
   if (is.null(colData)){
     colData <- DataFrame(depth = depths)
     mat<- lapply(seq_along(mat), function(x) { colnames(mat[[x]]) =codes[[x]]; mat[[x]]})
   } else {
     mat<- lapply(seq_along(mat), function(x) { colnames(mat[[x]]) =paste(codes[[x]], colData[[x,1]], sep = "_"); mat[[x]]})
     save <- character()
...
get_counts_from_beds <- function(beds, peaks, paired, colData = NULL) {
  ...
  results <- bplapply(seq_along(beds), function(i) {
    ...
    fragment <- readAlignmentFromBed(beds[i], paired = paired)
...
readAlignmentFromBed <- function(filename, paired, is_10x = FALSE) {
  ...
  if is_10x) {
    ...
    values(tmp_tmp) <- DataFrame(barcodes = tmp$barcodes)
    tmp <- tmp_tmp
  } else if (length(strand_col) == 1) {
