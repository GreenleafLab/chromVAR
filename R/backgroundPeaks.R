...
sampleBackgroundPeaks <- function(object, peak_set, counts_mat, niterations = 50){
  ...
  sample_mat = sparseMatrix(j = as.vector(object@background_peaks[peak_set,1:niterations]), 
                            ...
                            x=1, 
                            dims = c(niterations, counts_mat@npeak))
...
Rcpp::cppFunction('NumericVector mymahaC(NumericVector X1, NumericVector X2, NumericVector y, NumericMatrix s) {
  return (pow(X1 - y[0],2) * s(0,0) + (X1 - y[0])*(X2 - y[1])*(s(1,0) + s(0,1))  + pow(X2 - y[1],2)*s(1,1));
}')

# mymaha <- function(X, y, s){
#   x1y1 = X[,1] - y[1]
#   x2y2 = X[,2] - y[2]
#   out = (x1y1**2) * s[1,1] + x1y1*x2y2*(s[2,1] + s[1,2])  + (x2y2**2)*s[2,2]
#   return(out)
# }
...
getBackgroundPeakSets <- function(counts_mat, niterations = 50, window = 1000, BPPARAM = BiocParallel::bpparam(), for_set = TRUE){
  ...
  sample_peaks <- function(index){
    mdist <- mymahaC(X1, X2, norm_mat[index,], inv_cov_mat)
...
#' neighbors, niterations peaks are sampled from that background.
#' @export
getBackgroundPeakSets2 <- function(counts_mat, niterations = 50, window = 500, for_set = TRUE){
  stopifnot(inherits(counts_mat,"fragmentCounts"))
  #if bias not available...
  if ("bias" %ni% colnames(S4Vectors::mcols(counts_mat@peaks))){
    stop("Peaks must have metadata column named 'bias'. Compute using compute_bias.")
  ...
  sampled_peaks <- t(apply(nns, 1, function(x) reflected$replace(sample(x, niterations, replace = for_set))))
  rm(nns)
  background_peaks = backgroundPeaks(background_peaks = sampled_peaks, peaks = counts_mat@peaks)
  return(background_peaks)
}
