...
test_that("Can read annotations from multiple bed file",{
  res <- get_annotations(c(anno_file1, anno_file2), rowRanges = peaks)
  expect_equal(as.matrix(annotation_matches(res))[,1], c(TRUE, FALSE, FALSE))
  expect_equal(as.matrix(annotation_matches(res))[,2], c(FALSE, TRUE, TRUE))
  expect_equal(dim(res), c(3,2))
  expect_is(res, "RangedSummarizedExperiment")
})

test_that("Can read annotations from single bed file",{
  res <- get_annotations(anno_file3, rowRanges = peaks, column = 4)
  expect_equal(as.matrix(annotation_matches(res))[,1], c(TRUE, FALSE, FALSE))
  expect_equal(as.matrix(annotation_matches(res))[,2], c(FALSE, TRUE, TRUE))
  expect_equal(dim(res), c(3,2))
  expect_is(res, "RangedSummarizedExperiment")
...
test_that("Can get annotations from genomic ranges",{
  res <- get_annotations(anno_ranges, rowRanges = peaks)
  expect_equal(as.matrix(annotation_matches(res))[,1], c(TRUE, FALSE, FALSE))
  expect_equal(as.matrix(annotation_matches(res))[,2], c(FALSE, TRUE, TRUE))
  expect_equal(as.matrix(annotation_matches(res))[,3], c(FALSE, FALSE, FALSE))
  expect_equal(dim(res), c(3,3))
  expect_is(res, "RangedSummarizedExperiment")
})

test_that("Can get annotations from matrix",{
  res <- get_annotations(anno_mat, peaks)
  expect_equal(as.matrix(annotation_matches(res))[,1], c(TRUE, FALSE, FALSE))
  expect_equal(as.matrix(annotation_matches(res))[,2], c(FALSE, TRUE, TRUE))
  expect_equal(dim(res), c(3,2))
  expect_is(res, "RangedSummarizedExperiment")
...
test_that("Can get annotations from list",{
  res <- get_annotations(anno_list, npeaks = 3)
  expect_equal(as.matrix(annotation_matches(res))[,1], c(TRUE, FALSE, FALSE))
  expect_equal(as.matrix(annotation_matches(res))[,2], c(FALSE, TRUE, TRUE))
  expect_equal(as.matrix(annotation_matches(res))[,3], c(FALSE, FALSE, FALSE))
  expect_equal(dim(res), c(3,3))
