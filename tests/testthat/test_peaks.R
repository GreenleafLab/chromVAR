...
test_that("can read peaks file", {
  ...
  expect_equal(start(peaks), c(143913189,64977984,176032858,130506305,100183786,25315858,76585873,6255789,45973111,42772928,45996001))
  expect_equal(width(peaks) , rep(501, length(peaks)))
  expect_equal(as.character(seqnames(peaks)), c("chr1", "chr2", "chr2", "chr5", "chr7", "chr8", "chr10", "chr11", "chr17", "chr19", "chr19"))
})

test_that("can read peaks file with one extra column", {
  peaks <- get_peaks(peaks_file, extra_cols = c("name" = 4), sort = TRUE)
  expect_is(peaks, "GenomicRanges")
  expect_equal(colnames(mcols(peaks)), "name")
})

test_that("can read peaks file with several extra columns", {
  peaks <- get_peaks(peaks_file, extra_cols = c("name" = 4, "score" = 5), sort = TRUE)
  expect_is(peaks, "GenomicRanges")
...
test_that("reduce peaks works as expected", {
  ...
  expect_equal(filter_peaks(tmp2, ix_return = TRUE), c(1,2,4,5))
})
