...
To get just the plot of what is filtered, use `filter_samples_plot`

```{r, fig.width = 6, fig.height = 6}
#find indices of samples to keep
filtering_plot = filter_samples_plot(counts, interactive = TRUE)
filtering_plot
```
...
`plot_variability` takes the output of `compute_variability` and returns a plot of rank sorted annotation sets and their variability. By default, the plot will be interactive.  To turn off interactivity, use `interactive = FALSE`.
